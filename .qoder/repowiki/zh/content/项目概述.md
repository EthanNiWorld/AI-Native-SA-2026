...
## 更新摘要
**变更内容**
- 更新ai-knowledge-miner职责描述，强调处理notes/目录长期笔记的能力
- 增强对notes/目录作为长期维护个人笔记的说明
- 完善项目目录结构说明，突出长期笔记与一次性素材的区别
- 强化双Agent协同架构中知识沉淀的完整流程
...
- ai-knowledge-miner：负责将原始素材（inbox/）与长期笔记（notes/）提炼为脱敏、结构化的知识文档，沉淀到知识库相应目录，强调"提炼、沉淀、处理inbox、处理notes"。  
...
- 通过双Agent协同架构实现知识的持续积累与演进，支持短期素材处理与长期知识沉淀的双重需求。

## 项目结构
项目采用"素材-沉淀-索引"的三层结构：  
- inbox：原始素材收集区，由ai-native-expert在回答后自动生成，作为后续知识挖掘的输入。  
- notes：长期维护的个人笔记目录，包含日常思考、深度洞察与持续更新的内容，提炼后保留原文件。  
- knowledge：结构化知识库，按"AI General Notes（跨厂商通识）+ 各厂商/平台/应用/基础设施/方案"分类组织，配套标准化模板。  
- index.md：全局索引，提供"道-点-线-体"四层导航与模板参考，便于检索与复用。  
- archive：原始素材备份目录，用于历史数据归档与版本管理。

**更新**：notes/目录作为长期维护的个人笔记中心，与inbox的临时性形成鲜明对比。长期笔记强调持续性、深度性和个人化特点，为知识库提供深度洞察和实践经验积累。

```mermaid
graph TB
A["inbox<br/>原始素材一次性"] --> B["ai-knowledge-miner<br/>知识挖掘Agent"]
C["notes<br/>长期笔记持续维护"] --> B
D["ai-native-expert<br/>AI专家Agent"] --> A
B --> E["knowledge/<厂商>/<分类>/...<结构化文档>"]
E --> F["index.md<br/>全局索引"]
G["archive<br/>素材备份"] --> A
```
...
- ai-knowledge-miner（知识挖掘Agent）  
  - 职责：从inbox中抽取关键信息，进行脱敏与结构化处理，生成标准格式的知识文档，写入knowledge对应目录。**更新**：现需处理notes/目录的长期笔记，优先合并到现有文档避免重复创建，保留notes中的原始文件。  
  - 关键流程：接收素材/笔记 → 识别结构 → 脱敏清洗 → 模板填充 → 写入知识库 → 更新索引。  
...
- 处理：ai-knowledge-miner对inbox素材与notes长期笔记进行结构化处理，依据模板生成标准化文档。  
...
### ai-knowledge-miner（知识挖掘Agent）
- 设计理念  
  - 将"原始素材"与"长期笔记"转化为"结构化知识"，强调"脱敏、规范、可复用"。  
  - 以模板为中心，确保知识表达的一致性与完整性，降低后期维护成本。  
- 关键流程  
  - 素材识别：从inbox提取关键字段（如模型、厂商、场景、限制等），从notes识别深度洞察与个人经验。  
  - 脱敏清洗：去除敏感信息，统一格式与术语，保持长期笔记的原始价值。  
...
- 典型产出  
  ...
  - 行业解决方案与对比分析模板。  
  - **更新**：长期笔记的结构化整理，保留原始洞察的同时形成可复用的知识资产。  

**章节来源**
- [README.md:7-11](file://README.md#L7-L11)
- [_maas_template.md:1-65](file://knowledge/_maas_template.md#L1-L65)
- [_general_company_intro_template.md:1-249](file://knowledge/_general_company_intro_template.md#L1-L249)
- [index.md:60-78](file://index.md#L60-L78)
...
- 标准化流程  
  ...
  - **更新**：长期笔记与一次性素材的差异化处理流程，确保知识的持续演进。  
...
- **更新**：长期笔记与一次性素材的差异化处理策略，确保知识的持续积累与演进。  
...
- 组件耦合与协作  
  ...
  - **更新**：ai-knowledge-miner与notes之间存在间接依赖：长期笔记的结构化处理与知识沉淀的统一流程。  
- 外部依赖与集成点  
...
- [知识/_general_company_intro_template.md:1-249](file://knowledge/_general_company_intro_template.md#L1-L249)

## 性能考虑
- 知识挖掘吞吐与质量平衡  
  - 模板填充与脱敏处理的复杂度与文档规模成正比，建议批量处理与缓存常用模板。  
  - 对长文档进行分段处理与增量索引更新，降低索引重建开销。  
  - **更新**：长期笔记的处理需要考虑内容的持续演进特性，避免频繁重构已成熟的知识文档。  
- Agent循环稳定性  
...
- 内容安全检测与审核  
  ...
  - 视频合成API支持通过X-DashScope-DataInspection请求头禁用内容安全检测。  
- 素材与产物一致性  
  - 若发现知识库文档与inbox素材不一致，检查模板填充逻辑与脱敏规则，必要时回溯Agent生成链路。  
  - **更新**：长期笔记与知识库文档的一致性检查，确保洞察的准确传递与版本控制。  
- **更新**：notes/目录管理  
  - 长期笔记的版本管理与冲突处理策略  
  - notes与inbox的差异化处理流程验证  
  - 知识沉淀过程中的内容保留与去重机制  
...
- **更新**：目录结构  
  - inbox：原始素材收集区（一次性，提炼后自动归档）  
  - notes：长期维护的个人笔记目录（如Daily note，提炼后保留原文件）  
  - archive：原始素材备份目录  
  - knowledge：结构化的知识文档目录  
  - index.md：知识库全局索引（Skill必读）

**章节来源**
- [index.md:62-78](file://index.md#L62-L78)
- [_maas_template.md:1-65](file://knowledge/_maas_template.md#L1-L65)
- [_general_company_intro_template.md:1-249](file://knowledge/_general_company_intro_template.md#L1-L249)
- [README.md:13-19](file://README.md#L13-L19)